...
def split_documents(documents: list[Document]):
  text_splitter = RecursiveCharacterTextSplitter(chunk_size = 200, chunk_overlap = 20, length_function = len, is_separator_regex = False,)
...
def add_to_chroma(chunks: list[Document]):
  ...
  for chunk in tqdm(new_id_chunks):
    if chunk.metadata["id"] not in existing_ids: #and not (232 <= chunk.metadata["page"] <= 255) and not (3 <= chunk.metadata["page"] <= 8): #if not in existing set than  it is new
